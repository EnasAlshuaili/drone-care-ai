...
# --- [ 📊 Dashboard Page ] ---
    ...
    if df.empty:
        ...
        # Allows users to filter dashboard results
        # based on selected drone models
        # =========================================================
        ...
        # Displays high-level fleet performance indicators
        # =========================================================
        
        k1, k2, k3, k4 = st.columns(4)

        # Total number of drone missions
        k1.metric("Total Missions", len(f_df))

        # Average battery remaining across missions
        k2.metric(
            "Avg Battery Level",
            f"{int(f_df['Battery Remaining (%)'].mean())}%"
        )

        # Maximum recorded wind speed
        k3.metric(
            "Critical Wind Speed",
            f"{f_df['Wind Speed (m/s)'].max()} m/s"
        )
        ...
        # =========================================================
        # 3. TELEMETRY DISTRIBUTION ANALYSIS
        # Visualizes how telemetry values vary
        # between successful and failed flights
        # =========================================================
        ...
        # =========================================================
        # 4. MISSION STATUS OVERVIEW
        # Pie chart + drone model success comparison
        # =========================================================
        ...
        with col_left:
            ...
            # Count occurrences of each flight status
            status_counts = (
                f_df['Flight Status']
                .value_counts()
                .reset_index()
        ...
        with col_right:
            ...
            # Group missions by drone model and flight status
            grouped_model = (
                f_df.groupby(['Drone Model', 'Flight Status'])
                .size()
        ...
        # and identifies possible failure patterns
        # =========================================================
...
# --- [ 🚁 Manual Input Page ] ---
    ...
    # =========================================================
    # Display last prediction stored in session state (if available)
    # This allows users to view their previous result without re-running the model
    # =========================================================
    if st.session_state.last_prediction:
        with st.expander("📋 Last Prediction Result"):
            if st.session_state.last_prediction.get('status') == 'Completed':
                st.success(
                    f"✅ {st.session_state.last_prediction['status']} - "
                    f"Confidence: {st.session_state.last_prediction['confidence']:.2%}"
                )
            else:
                st.error(
                    f"🚨 {st.session_state.last_prediction['status']} - "
                    f"Risk: {st.session_state.last_prediction['risk']:.2%}"
                )
    
    # =========================================================
    # User Input Form for Mission Parameters
    # Collects all required features for model prediction
    # =========================================================
    with st.form("risk_form"):
        col1, col2, col3 = st.columns(3)
        
        # ---------------- COLUMN 1: Environmental Conditions ----------------
        with col1:
        ...
        # ---------------- COLUMN 3: Mission & Payload Details ----------------
        with col3:
        ...
        # =========================================================
        # Pre-submission validation (Safety checks)
        # =========================================================
        if batt < 10:
        ...
        # =========================================================
        # Model execution triggered after form submission
        # =========================================================
        if submit:

            # Safety constraint: prevent overload condition
            if act_w > max_w:
                st.error("Cannot proceed: Weight exceeds drone capacity!")

            # Ensure model and scaler files are loaded
            elif ann_model is None or data_scaler is None:
                st.error("Missing Files: Ensure 'ann2_model.h5' and 'scaler2.pkl' exist in project folder.")

            else:
                # =========================================================
                # Simulated AI processing steps (UI progress feedback)
                # =========================================================
                ...
                # =========================================================
                # Feature vector construction (must match training order exactly)
                # =========================================================
                raw_feats = [
                ...
                # Feature scaling (normalization for ANN input)
                ...
                # Neural network prediction output (risk probability)
                ...
                # =========================================================
                # Final result display with safety interpretation
                # =========================================================
                if batt <= 5 or prediction_prob > 0.5:
                    ...
                    st.info("📝 Recommended action: Land immediately and inspect the drone.")
...
# --- [ 📈 Model Performance Page ] ---
    ...
    m_col1.metric("Model Accuracy", "98%", "Top Tier")
    m_col2.metric("Precision", "96%", "+1.4%")
    m_col3.metric("F1-Score", "96%", "Balanced")
    ...
    with c1:
        ...
        <div style='background:rgba(0, 242, 255, 0.05); padding:20px; border-radius:15px;'>
            <p> Artificial Neural Networks (ANN) are deep learning models inspired by the human brain. 
                    They consist of interconnected layers of nodes (neurons) that process and learn from data by adjusting weights and biases. 
                    ANN is widely used for solving complex problems where traditional methods may struggle..</p>
            <br>
            <p><strong>Why ANN is Powerful:</strong></p>
            <ul>
                    <li>Can capture complex and non-linear relationships in data</li>
                    <li>Works well with large and high-dimensional datasets</li>
                     <li>Learns patterns automatically without explicit programming</li>
                    <li>Adapts and improves performance with more training data</li>
                    <li>Handles noisy and real-world data effectively</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    with c2:
...
# --- [ 👥 About Us Page ] ---
    ...
    st.markdown(f"""
        <div style='text-align:center; padding: 25px 10px;'>
            <h1 style='font-size: 48px; margin-bottom: 10px;'>
                🛡️ GRADUATION PROJECT: <span style='color:{ACCENT_CYAN};'>DRONE CARE</span>
            </h1>
            <p style='color:{ACCENT_CYAN}; font-size: 20px;'>
                University of Technology and Applied Sciences (UTAS) | 2026
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    # --- Mission Section ---
    col_text, col_gfx = st.columns([1.3, 0.7])
    ...
    with col_text:
        ...
        <b>DroneCare</b> is an AI-powered predictive system designed to detect potential drone failures
        <b>before they occur</b> by analyzing real-time flight data.
        </p>

        <p style='font-size: 17px; line-height: 1.7;'>
        The system identifies early warning signals and notifies operators immediately,
        helping prevent crashes and improve operational safety.
        </p>
    ...
    with obj1:
        st.markdown("""
        <div style='background:#001d3d; padding:20px; border-radius:15px; text-align:center; height:180px;'>
            <h2>🛡️</h2>
            <h4>Asset Protection</h4>
            <p>Reduce damage to infrastructure and equipment.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with obj2:
        ...
        <div style='background:#001d3d; padding:20px; border-radius:15px; text-align:center; height:180px;'>
            <h2>👥</h2>
            <h4>Public Safety</h4>
            <p>Prevent drone crashes in populated areas and protect lives.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with obj3:
        ...
        <div style='background:#001d3d; padding:20px; border-radius:15px; text-align:center; height:180px;'>
            <h2>🧠</h2>
            <h4>AI Prediction</h4>
            <p>ANN model achieving high accuracy for failure detection.</p>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("---")

    st.caption("© 2026 DroneCare Team - UTAS | Powered by Artificial Intelligence 🚀")